...
''' -------接口性能测试配置-------'''
method = "post" # 接口类型
url = 'http://172.13.2.163/vdinapi/face/v1/detect'  # 接口地址
data = {
    "appKey": "98e997dee93a9ba21a46fc897d633bfc"
    , "appSecret": "eb63d2ab9abe36628579b29a5ca7672b"
    }    # 接口参数
files = {"imageData":  ('20190315153956.png', open('D:/work/face/img/20190315153956.png', 'rb').read(), 'image/png')}
headers={"Content-type": "multipart/form-data","Accept": "text/plain"}

thread_num = 50000  # 线程数
one_work_num = 1    # 每个线程循环次数
loop_sleep = 1  # 每次请求时间间隔
...
class CreateThread:
    ...
    def thread_api(cls):
        ...
        try:
            if method == "post":
                results = requests.post(url=url, data=data,files=files)
            if method == "get":
                results = requests.get(url=url, data=data,files=files,headers=headers)
            print(results.json())
            #print(results,results.status_code)
            return results
