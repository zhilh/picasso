...
class Main_File_handle():
    '''
            对文件操作的类;
            @ file_path : 文件路径；不能有反斜杠 '\' ；建议格式: D:/pytest/files/test.ini'
    '''
    ...
    def copy_file(self, targetDir):
        if self.examine_file() == True:
            ...
            if targetDir_list == input_file.split("\\"):
                ...
                file_new_name = self.file_path.split(".")[0]+"_Maincopy."+self.file_path.split(".")[1]
                print(file_new_name)
...
class Json_File_Main(Main_File_handle):
    """
        Json 文件 增删改查
        继承 Main_File_handle
    """
    def __init__(self, file_path):
        #先继承，再构造
        #继承父类的构造方法
        Main_File_handle.__init__(self, file_path)

    def class_name(self):
        print("Json_File_Main --> Main_File_handle.")
...
class Ini_File_Main(Main_File_handle):
    """
        ini 文件 增删改查
        继承 Main_File_handle
    """
    def __init__(self, file_path, cfg_debug=False ):
        Main_File_handle.__init__(self, file_path)
        #super(Main_File_handle, self).__init__()
        #self.file_path = file_path
    ...
    def class_name(self):
        print("Ini_File_Main --> Main_File_handle.")
...
c_test = 'D:/py_test/yibiao_Auto/unitys/test_json1_Maincopy_Maincopy.json'
ini_test = 'D:/py_test/yibiao_Auto/unitys/test.ini'

file = "D:/py_test/yibiao_Auto/unitys/test_json.json"

txt_file='D:\Python\projects\picasso\Logs/testLog.log'
ini_file='D:\Python\projects\picasso\config/config.ini'

#f = Main_File_handle(file)
#json =     Json_File_Main(a_file_path)
#ini = Ini_File_Main(ini_file)
txt = Main_File_handle(txt_file)
data = txt.read_all_data()
print(data)
