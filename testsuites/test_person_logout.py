...
from pageobject.person_logout_page import pageElements as centerPage,MAIN_IFRAME
...
class Industry_Search(unittest.TestCase):
    '''人员注销'''
    #装载测试数据
    execlName=os.path.join(getcwd.get_cwd(),"data\\person_logout_data.xlsx")
    ...
    def setUp(self):        
        ...
        self.menu_page.go_person_logout()   #点击菜单到 人员注销 功能链接
    ...
    def test_index(self,test_data):
        ...
        search_dict={
            ...
            'phone_number':test_data['phone_number'].strip(), #联系电话           
        ...
        if search_dict['place_name']=='' and search_dict['industry_name']=='' and search_dict['person_name']==''  \
         ...
         and search_dict['apply_date_from']=='' and search_dict['apply_date_to']=='':
            self.assertTrue(1==2,'查询条件（所属企业，所属行业，人员姓名，性别，证件号码，联系电话，备案时间）为空，不做注销操作！')        
        ...
        for i in range(len(table_data)):
            ...
            if int(len(table_data[i])>1):
                okFlag = 0  
                if table_data[i][0].lower().find(search_dict['person_name'].lower()) != -1 and len(search_dict['person_name'])>0:#验证输入了从业人员姓名时，查询结果包含输入的从业人员姓名信息
                    okFlag=1
                if search_dict['person_sex'].lower() == table_data[i][1].lower() and len(search_dict['person_sex'])>0:#验证输入了从业人员性别时，查询结果等于输入的从业人员性别
                    okFlag=1
                if search_dict['person_id_card'].lower() == table_data[i][2].lower() and len(search_dict['person_id_card'])>0:#验证输入了从业人员证件号码时，查询结果等于输入的从业人员证件号码
                    okFlag=1
                if table_data[i][3].lower() == search_dict['phone_number'].lower() and len(search_dict['phone_number'])>0:#验证输入了联系电话时，查询结果等于输入的联系电话信息
                    okFlag=1
                if table_data[i][4].lower().find(search_dict['place_name'].lower()) != -1 and len(search_dict['place_name'])>0:#验证输入了企业名称时，查询结果包含输入的企业名称信息
                    okFlag=1
                if search_dict['industry_name'].lower().find(table_data[i][5].lower()) != -1 and len(search_dict['industry_name'])>0:#验证输入了所属行业时，查询结果包含输入的所属行业信息
                    okFlag=1
                if  table_data[i][6].lower() >= search_dict['apply_date_from'].lower() and len(search_dict['apply_date_from'])>0:#验证输入了备案开始时间时，查询结果》=备案开始时间
                    okFlag=1
                if table_data[i][6].lower() <= search_dict['apply_date_to'].lower()  and len(search_dict['apply_date_to'])>0:#验证输入了备案结束时间时，查询结果《=备案结束时间
                    okFlag=1
                if okFlag == 0:
        ...
        for i in range(len(table_data)):
            if int(len(table_data[i])>1 ):
                ...
                if self.page.logout_success_name_is_enabled():
                    self.assertTrue(1==1,'人员注销成功')
                    log.info("人员注销成功，操作员：%s，%s；注销信息：%s，%s，%s，%s" %(globals()['node_name'],globals()['login_name'],table_data[i][0],table_data[i][2],table_data[i][3],table_data[i][4]))
                else:
                    self.assertTrue(1==2,"人员注销失败：%s"%self.page.get_fail_text())
                break
