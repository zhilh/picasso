...
from pageobject.place_query_page import pageElements as centerPage,MAIN_IFRAME
...
class Industry_Search(unittest.TestCase):
    '''企业查询'''
    #装载测试数据
    execlName=os.path.join(getcwd.get_cwd(),"data\\place_query_data.xlsx")
    ...
    def setUp(self):        
        ...
        self.menu_page.go_place_query()   #点击菜单到 行业查询 功能链接
    ...
    def test_index(self,test_data):
        ...
        2、根据显示数据，只做部分查询条件验证： 场所名称、所属行业、企业法人、企业类型
        3、其他查询条件犹豫没有对应结果显示在页面，暂时无法验证
        4、查询结果中只要有一条数据验证失败，则这条case不通过，只有全部数据验证通过，这条case才通过
        5、这里只验证查询出来的数据是否正确，没有查询到数据的情况无法验证是否正确
        '''
        if search_dict['place_name']=='' and search_dict['industry_name']=='' and search_dict['person_name']=='' and search_dict['place_type']=='所有' and search_dict['region_name']=='':
            self.assertTrue(1==2,'验证条件（场所名称、所属行业、企业法人、企业类型、管辖机构）为空，不做验证！')        
        ...
        for i in range(len(table_data)):
            ...
            if int(len(table_data[i])>1):
                ...
                if table_data[i][2].lower().find(search_dict['person_name'].lower()) != -1 and len(search_dict['person_name'])>0:#验证输入了企业法人时，查询结果包含输入的企业法人信息
                    okFlag=1
                if search_dict['place_type'].lower().find(table_data[i][3].lower()) != -1 and len(search_dict['place_type'])>0:#验证输入了企业类型时，查询结果包含输入的企业类型信息
                    okFlag=1
