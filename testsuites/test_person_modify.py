...
from pageobject.person_modify_page import pageElements as centerPage,MAIN_IFRAME
    ...
    def test_index(self,test_data):
        ...
        if search_dict['place_name']=='' and search_dict['industry_name']=='' and search_dict['person_name']==''  \
         ...
         and search_dict['apply_date_from']=='' and search_dict['apply_date_to']=='':
            self.assertTrue(1==2,'查询条件（所属企业，所属行业，人员姓名，性别，证件号码，联系电话，备案时间）为空，不做变更操作！')        
        ...
        for i in range(len(table_data)):
            if int(len(table_data[i])>1 ):
                ...
                if self.page.success_name_is_enabled():
                    ...
                    log.info("人员变更成功，操作员：%s，%s；变更信息：%s，%s，%s，%s" %(globals()['node_name'],globals()['login_name'],table_data[i][0],table_data[i][2],table_data[i][3],table_data[i][4]))
                else:
                    self.assertTrue(1==2,"人员变更失败：%s"%self.page.get_fail_text())
                break
