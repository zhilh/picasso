...
apiUrl = 'http://172.13.2.163/vdinapi/face/v1/detect'
...
data={
    "appKey": appKey
     ,"appSecret": appSecret
     ,'imageBase64': getImgBase64(fileUrl)
    }
headers={
...
async def post_api(session, i):
    ...
    try:
        async with session.post(url=apiUrl, data=data,headers=headers) as response:
