...
class pageElements(object):
    ...
    def __init__(self, driver):
        ...
        self.modify_button='xpath=>//*[@class="js-search-results-tbody"]/tr[%d]/td[9]/a'#变更按钮，每条数据的变更按钮定位根据tr值
        
        #人员信息页面
        self.save_button='class=>js-submit-filings'#保存按钮
        self.modify_success_name='class=>js-modal-success-name'#成功后提示框
        self.modify_fail_name='class=>js-modal-fail-name'#失败后提示框
    ...
    def select_modify_button(self,val):#选择点击变更按钮
        #print(self.modify_button%val)
        self.driver.click(self.modify_button%val)        
    
    def click_save_button(self):#点击人员信息页面，保存按钮
        self.driver.click(self.save_button) 
                
    def success_name_is_enabled(self):#提交后，成功信息是否可见
        return self.driver.findElement(self.modify_success_name)
    
    def get_fail_text(self):#失败信息
        return self.driver.getText(self.modify_fail_name)
